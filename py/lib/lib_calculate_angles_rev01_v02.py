...
class CalculateAngles:
    def __init__(self, _connectDB, _limDataArr, _limWinArr, _opt) -> None:
        ...
        self.ICM20948 = ICM20948( gfs=_opt.GyroFullScale\
                                 ,gsr=_opt.GyroSampleRate\
                                 ,glp=_opt.GyroLowPass\
                                 ,afs=_opt.AccFullScale\
                                 ,asr=_opt.AccSampleRate
                                 ,alp=_opt.AccLowPass\
                                 ,msm=_opt.MagSampleRate\
                                 ,isr=_opt.Isr) # инстанцировать модуль для работы с датчиком IMU ICM20948
