...
class ICM20948:
    ...
    def i2c_master_enable(self):
        ...
        self.bank(0)
        temp = self.read(ICM20948_USER_CTRL)
        self.write(ICM20948_USER_CTRL, temp | 0b00100000)  # |0x20
        time.sleep(0.01)
        # Master clock set | 0x07 - 400kHz, page 68
        self.bank(3)
        self.write(ICM20948_I2C_MST_CTRL, 0b111)   #0x07
        time.sleep(0.01)
        self.bank(0)
        self.write(ICM20948_LP_CONFIG, 0b01110000) #works
        # self.write(ICM20948_LP_CONFIG, (1<<4)) #doesnt work
        time.sleep(0.01)
        self.bank(3)
        # I2C_MST_ODR_CONFIG: 136Hz, page 68
    ...
    def __init__(self, i2c_addr=I2C_ADDR, i2c_bus=None):
        ...
        # установка ODR и dlpf гиро
        # temp = self.read(ICM20948_GYRO_CONFIG_1)
        # self.write(ICM20948_GYRO_CONFIG_1, 0b00111011) #500dps, 50hz dlpf on
        # self.write(ICM20948_GYRO_CONFIG_2, 0b00111000) #self-tests on, no averaging 
        self.set_gyro_sample_rate(50)
        self.set_gyro_low_pass(enabled=True, mode=3)
        self.set_gyro_full_scale(500)

        # установка ODR и dlpf акселерометра
        # self.write(ICM20948_ACCEL_CONFIG, 0b00011001)   # 50hz dlpf on
        self.set_accelerometer_sample_rate(50)
        self.set_accelerometer_low_pass(enabled=True, mode=3)
        self.set_accelerometer_full_scale(2)

        self.bank(0)
        self.write(ICM20948_INT_PIN_CFG, 0b00110000)
        #self.write(ICM20948_INT_PIN_CFG, 0b00110010) - возможно необходимо использовать эту команду
