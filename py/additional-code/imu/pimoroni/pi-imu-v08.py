...
class ICM20948:
    ...
    def read_magnetometer_data(self, timeout=1.0):
        ...
        x, y, z, st2 = struct.unpack("<hhhh", bytearray(data)) # распаковать данные - 4 signed short справа налево
    ...
    def read_acc_gyro_mag_data(self):   #метод в процессе разработки
        self.bank(0)
        data = self.read_bytes(ICM20948_ACCEL_XOUT_H, 22)

        ax, ay, az, gx, gy, gz, tmprt, mx, my, mz, st2 = struct.unpack(">hhhhhhhhhhh", bytearray(data))
