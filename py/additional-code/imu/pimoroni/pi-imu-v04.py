...
class ICM20948:
    ...
    def trigger_mag_io(self):
        ...
        time.sleep(0.005)
        #fix self.write(ICM20948_USER_CTRL, user)
        self.write(ICM20948_USER_CTRL, user & ~0x20)
    ...
    def mag_write(self, reg, value):
        ...
        self.bank(3)
        self.write(ICM20948_I2C_SLV0_ADDR, AK09916_I2C_ADDR) # addr=0x0c # Write one byte
        self.write(ICM20948_I2C_SLV0_REG, reg)
        self.write(ICM20948_I2C_SLV0_DO, value)
        time.sleep(0.05)
        self.write(ICM20948_I2C_SLV0_CTRL, 0x80|0x01)
        time.sleep(0.05)
    ...
    def mag_read(self, reg, length=1):
        ...
        self.write(ICM20948_I2C_SLV0_ADDR, AK09916_I2C_ADDR | 0x80)   #передается адрес магнитометра на чтение
        self.write(ICM20948_I2C_SLV0_REG, reg)                        #
        self.write(ICM20948_I2C_SLV0_DO, 0xff)
        time.sleep(0.05)
        self.write(ICM20948_I2C_SLV0_CTRL, 0x80 | length)  # Read 1 byte
        time.sleep(0.05)
        # return self.read(ICM20948_EXT_SLV_SENS_DATA_00)

    def read_magnetometer_data(self, timeout=1.0):

        self.mag_read(AK09916_HXL, 8)
        data = self.read_bytes(ICM20948_EXT_SLV_SENS_DATA_00, 8)
        return data
        print(data)
    ...
    def __init__(self, i2c_addr=I2C_ADDR, i2c_bus=None):
        ...
        # if not self.mag_read(AK09916_WIA, 2) == AK09916_CHIP_ID:
        #     raise RuntimeError("Unable to find AK09916")
        self.mag_init()
        self.mag_read(AK09916_WIA, 1)
        time.sleep(0.01)
        print('status')
        s = self.read(ICM20948_EXT_SLV_SENS_DATA_00)
        print(s)
        
        self.mag_read(AK09916_HXL, 8)
        # print(self.read(AK09916_CNTL2))
    ...
    def i2cMasterPassthrough(self, passthrough):
        ...
        self.bank(0)
        register = self.read(self._addr, ICM20948_INT_PIN_CFG)
        ...
        # Write register
        self.bank(0)
        return self.write(self._addr, ICM20948_INT_PIN_CFG, register)	
    ...
    def mag_init(self):
        ...
        # Master I2C enable
        temp = self.read(ICM20948_USER_CTRL)
        self.write(ICM20948_USER_CTRL, temp | 0b100000)  # |0x20
        time.sleep(0.01)
        ...
        time.sleep(0.01)
        self.bank(0)
        self.write(ICM20948_LP_CONFIG, 0b1000000) #0x40
        self.bank(3)
        # I2C_MST_ODR_CONFIG: 1.1kHz, page 80
        self.write(ICM20948_I2C_MST_ODR_CONFIG, 0b11)   #0x03
        time.sleep(0.01)
        # mag reset
